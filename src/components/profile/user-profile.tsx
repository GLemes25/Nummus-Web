...
type SettingSection = {
  ...
  iconColor: string;
  items: { label: string; value?: string; toggle?: boolean; toggled?: boolean }[];
};
...
const UserProfile = ({ onLogout }: UserProfileProps) => (
  <div className="p-6 pt-7 max-w-[720px]">
    <h1 className="text-foreground m-0 mb-7 tracking-tight text-2xl font-bold">Profile & Settings</h1>
    ...
    <motion.div
      ...
      style={{ background: 'linear-gradient(135deg, #18181b 0%, #1e1030 100%)' }}
    >
      ...
      <div className="relative">
        <div className="w-[72px] h-[72px] rounded-full flex items-center justify-center text-[26px] text-foreground font-extrabold border-[3px] border-muted shrink-0" style={{ background: 'linear-gradient(135deg, #7C3AED, #5B21B6)' }}>
          AJ
        </div>
        <Button variant="secondary" size="icon" className="absolute bottom-0 right-0 w-6 h-6 rounded-full bg-muted border-2 border-card text-muted-foreground p-0">
          <Camera size={11} />
        </Button>
      </div>
      ...
      <div className="flex-1">
        <div className="text-foreground font-bold text-lg tracking-tight mb-0.5">Alex Johnson</div>
        <div className="text-muted-foreground text-sm mb-2.5">alex@example.com</div>
        <div className="inline-flex items-center gap-1.5 bg-gold/10 border border-gold/30 rounded-full px-3 py-1">
          <Crown size={12} className="text-gold" />
          <span className="text-gold text-xs font-semibold tracking-wide">NUMMUS PRO</span>
        </div>
      </div>
      ...
      {quickLinks.map((link, i) => {
        ...
        return (
          <Button
            ...
            style={{ borderBottom: i < quickLinks.length - 1 ? '1px solid #1f1f22' : 'none' }}
          >
            <div className="w-9 h-9 rounded-[9px] flex items-center justify-center shrink-0" style={{ backgroundColor: link.color + '18' }}>
              <Icon size={16} style={{ color: link.color }} />
            </div>
            <span className="text-foreground text-sm flex-1 text-left">{link.label}</span>
      ...
      return (
        ...
        >
          <div className="flex items-center gap-2 mb-2 pl-1">
            <div className="w-[22px] h-[22px] rounded-[6px] flex items-center justify-center" style={{ backgroundColor: section.iconColor + '20' }}>
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            {section.items.map((item, ii) => (
              <div
                ...
                style={{ borderBottom: ii < section.items.length - 1 ? '1px solid #1f1f22' : 'none' }}
              >
    ...
    >
      <div className="flex flex-col gap-2.5">
        ...
        <Button variant="outline" className="w-full text-zinc-700 border-border text-sm">
          Delete account permanently
        </Button>
      </div>

      <div className="text-center mt-6 mb-2">
        <span className="text-zinc-700 text-xs">Nummus Premium v2.4.1 · </span>
        <Button variant="link" size="sm" className="text-zinc-600 p-0 h-auto text-xs">Privacy Policy</Button>
        <span className="text-zinc-700 text-xs"> · </span>
        <Button variant="link" size="sm" className="text-zinc-600 p-0 h-auto text-xs">Terms of Service</Button>
      </div>
    </motion.div>
  </div>
);
