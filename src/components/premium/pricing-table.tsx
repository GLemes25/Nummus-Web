  ...
  <div className="p-6 pt-7 max-w-[960px]">
    ...
    <div className="text-center mb-12">
      ...
      >
        ...
        <span className="text-gold text-xs font-semibold tracking-wide">NUMMUS PREMIUM</span>
      </motion.div>
      <motion.h1
        initial={{ opacity: 0, y: 14 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        Upgrade to Pro and unlock AI-powered insights, unlimited wallets, and advanced analytics.
      </motion.p>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-14">
      ...
      >
        <div className="mb-7">
          ...
          <div className="flex items-end gap-1.5 mb-2">
            <span className="text-foreground text-[42px] font-extrabold tracking-tight">$0</span>
            <span className="text-muted-foreground text-sm pb-1.5">/month</span>
          </div>
          <p className="text-muted-foreground text-sm">Get started with essential budgeting tools, no credit card required.</p>
        </div>

        <Button variant="outline" className="w-full mb-7 rounded-xl text-zinc-400 border-border">
          Current Plan
        </Button>

        <div className="flex-1">
          <div className="text-muted-foreground text-xs tracking-[0.8px] mb-3.5">INCLUDED FEATURES</div>
          <div className="flex flex-col gap-3">
            {freeFeatures.map((f, i) => (
              <div key={i} className="flex items-center gap-2.5">
                <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: f.included ? 'rgba(16,185,129,0.12)' : 'rgba(113,113,122,0.08)' }}>
                  {f.included ? <Check size={11} className="text-income" /> : <X size={10} className="text-zinc-600" />}
                </div>
                <span className={`text-sm ${f.included ? 'text-zinc-300' : 'text-zinc-600'}`}>{f.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <motion.div
        ...
        style={{ boxShadow: '0 0 40px rgba(191,160,113,0.1), 0 0 80px rgba(124,58,237,0.05)' }}
      >
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(145deg, rgba(191,160,113,0.04) 0%, rgba(124,58,237,0.04) 100%)' }} />
        ...
        <div className="mb-7 relative">
          ...
          <div className="flex items-end gap-1.5 mb-2">
            <span className="text-foreground text-[42px] font-extrabold tracking-tight">$9</span>
            <span className="text-muted-foreground text-sm pb-1.5">/month</span>
            <span className="bg-income/10 text-income text-xs font-semibold px-2 py-0.5 rounded-md border border-income/20 pb-2">SAVE 30% ANNUAL</span>
          </div>
          <p className="text-muted-foreground text-sm">Everything in Free, plus AI insights, unlimited wallets, and priority support.</p>
        </div>
        ...
        <div className="flex-1 relative">
          <div className="text-muted-foreground text-xs tracking-[0.8px] mb-3.5">EVERYTHING IN FREE, PLUS</div>
          <div className="flex flex-col gap-3">
            {proFeatures.map((f, i) => (
              <div key={i} className="flex items-center gap-2.5">
                <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 border border-brand/30" style={{ backgroundColor: 'rgba(124,58,237,0.15)' }}>
    ...
    {/* Testimonials */}
    <div className="mb-12">
      <h3 className="text-muted-foreground text-center mb-6 tracking-wide text-xs font-semibold">LOVED BY THOUSANDS OF USERS</h3>
        ...
        {testimonials.map((t, i) => (
          ...
          >
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-[34px] h-[34px] rounded-full flex items-center justify-center text-xs text-foreground font-bold shrink-0" style={{ background: `linear-gradient(135deg, ${t.color}, ${t.color}88)` }}>
                {t.avatar}
              </div>
              <div>
    ...
    <div className="bg-card border border-border rounded-2xl p-8 text-center">
      ...
      <h3 className="text-foreground mb-2 font-semibold">30-day money-back guarantee</h3>
      <p className="text-muted-foreground text-sm mx-auto max-w-[400px]">
        Try Nummus Pro risk-free for 30 days. If you&apos;re not completely satisfied, we&apos;ll refund your payment — no questions asked.
      </p>
    </div>
  </div>
);
